...
public class CharacterMovement : MonoBehaviour
{
    ...
    [SerializeField] private bool m_AirControll = false;                                     // 점프 중에도 조작 할 수 있는지 여부
    ...
    private float limiFallSpeed = 25f;                                                      // 낙하 속도 제한
    ...
    private CharacterStaus status;

    private void Awake()                                // 컴포넌트들을 가져와서 셋팅한다
    {
        m_Rigidbody2D = GetComponent<Rigidbody2D>();
        animator = GetComponent<Animator>();
        status = GetComponent<CharacterStaus>();
    ...
    private void FixedUpdate()                          // FixedUpdated에서 이동 구현을 한다.
    {
        bool wasGrounded = m_IsGrounded;                // 땅 체크 값을 이전 변수에 넣어서 매 프레임마다 갱신
        m_IsGrounded = false;

        // GroundCheck 위치에서 땅으로 지정된 것과 겹치는지 확인
        Collider2D[] groundCloiiders = Physics2D.OverlapCircleAll(m_GroundCheck.position, k_GroundedRadius, m_WhatIsGround);

        for (int i = 0; i < groundCloiiders.Length; i++)
        {
            if (groundCloiiders[i].gameObject != gameObject)
            {
                m_IsGrounded = true;
                if (!wasGrounded)
                {
        ...
        if (!m_IsGrounded) OnFallEvent.Invoke();                // 땅이 아닐때 떨어지고 있가는 이벤트를 호출
        ...
        Collider2D[] wallCloiiders = Physics2D.OverlapCircleAll(m_WallCheck.position, k_GroundedRadius, m_WhatIsGround);

        for (int i = 0; i < wallCloiiders.Length; i++)
        {
            if (wallCloiiders[i].gameObject != gameObject)
            {
                m_IsWall = true;
                if (!m_IsGrounded && !status.isDashing && m_Rigidbody2D.velocity.y < 0)
                {
                    m_IsWallSlidding = true;
                    if (!wasWallSlidding)               // 이전 프레임이 벽이 아니였다면 체크히여 돌려서 벽을 슬라이딩 하고 검출 하게 한다.
                    {
    ...
    public void Move(float move, bool jump, bool dash)
    {
        if (status.canMove)             // 클래스에서 이동이 가능하다고 하면
        {
            ...
            else if (m_IsGrounded || m_AirControll)      // 플레이어가 땅에 닿았거나 공중 제어가 켜져있을때
            {
                if (m_Rigidbody2D.velocity.y < -limiFallSpeed) m_Rigidbody2D.velocity = new Vector2(m_Rigidbody2D.velocity.x, -limiFallSpeed);  // 속도제한
            ...
            {
                animator.SetBool("isJumping", true);                                // 애니메이터에서 점프 상태 전달
            ...
            else if (m_IsWallSlidding && !m_IsWall && status.checkTimer.GetRemainingTime() <= 0)    // 벽 슬라이딩 중이고 벽이 끝났을때
            {
                m_IsWallSlidding = false;
                animator.SetBool("isWallSliding", false);
                oldWallSlidding = false;
