...
{
    private AnimationClip animationClip;                                            // 선택괸 애니메이션 클립
    private List<SpriteInfo> spriteInfoList = new List<SpriteInfo>();               // 스프라이트정보를 저장할 리스트

    [MenuItem("Window/Animation Sprite Extractor")]                                 // 메뉴에서 Animation Sprite Extractor 항목을 추가
    public static void ShowWIndow()
    {
        GetWindow<AnimationSpriteExtractor>("Animation Sprite Extractor");          // 에디터 창 열기
    }
        ...
        if (animationClip != null)  // 애니메이션 클립이 설정된 경우
        {
            ...
            if (spriteInfoList.Count > 0)   // 스프라이트 정보 리스트가 비어있지 않은 경우, 리스트의 내용을 표시
            {
                ...
                foreach (var spriteInfo in spriteInfoList)
                {
                    ...
                    GUILayout.Label("Time :", GUILayout.Width(50));                         // Time 테이블
    ...
    {
        spriteInfoList.Clear();     // 기존 스프라이트 초기화
        var bindings = AnimationUtility.GetObjectReferenceCurveBindings(Clip);  // 애니메이션 클립에서 GetObject Reference Curve 바인딩을 가져옴
